...
$root = admin();
if(isset($LOGIN) && $LOGIN){ if(empty($_SESSION['user'])) die (include('login.php'));}
if(!isset($_GET['rep']) || $_GET['rep'] =='')
	$_GET['rep']=$BASE_DIR;
//Sécu
if(! check_folder($_GET['rep'])){
	$_GET['rep']=$BASE_DIR;
...
<header>
	...
	<div class="header_right">
		<form method="GET" action="index.php">
			<select name="rep" onChange="this.form.submit()" style="margin-right:2px;margin-top:3px;">
				<option>--- Selection dossier ---</option>
				<?php foreach( get_folders() as $folder ):?>
				<option value="<?php echo $BASE_DIR.'/'.$folder ?>"><?php echo $folder ?></option>
				<?php endforeach; ?>
			</select>
		</form>
	</div>
...
while ($data = mysql_fetch_array($req)){
	...
	echo '><tr>';
	if(isset($data['id_movie']) &&$data['id_movie'] != '0' and $data['id_movie'] != '0-0-0') echo '<td><a href="#null" rel="'.urlencode($data['link']).'" class="opener movielist"><img src="images/info.png" alt="Info"></a></td>';
	echo '<td><a href="';
	...
	echo '</tr>
	<tr>';
	if(isset($data['id_movie']) && $data['id_movie'] != '0' and $data['id_movie'] != '0-0-0') echo '<td>'.infos.'</td>';
	echo '<td>'.link.'</td>';
	...
	echo '" class="movielist" title="'.$data['link'].'">'.length($data['name'],22).'</a></h5>';
	if(isset($data['year'])) echo '<p>'.$data['year'].'</p>';
	echo '</div>';
