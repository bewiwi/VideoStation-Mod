...
session_start();
if(isset($_GET['action']) && $_GET['action'] == 'mod' and isset($_POST['pass'])){
	...
	$seri = $_POST['seriesdir'];
	$seri = explode(',',$seri);
	$seri_array = 'array(';
	for($i=0;$i<count($seri);$i++){
	$seri_array .= '"'.$seri[$i].'"';
	if($i != (count($seri)-1)) $seri_array .= ',';
	}
	$seri_array .= ')';
...
<div id="tabs" style="width:85%;margin-left:auto;margin-right:auto;min-height:400px;max-height:600px;overflow:auto;">
	...
	<div id="tabs-1">
		...
		<table>
			...
			<tr>
				<td><?php echo sqllogin;?></td><td><input type="text" name="loginsql" value=<?php echo "\"".$LOGIN_SQL."\"";?>></td>
			</tr>
			<tr>
			...
			<tr>
				<td><?php echo basedir;?></td><td><input type="text" name="basedir" value=<?php echo "\"".$BASE_DIR."\"";?>></td>
			</tr>
			
			<tr>
				<td><?php echo seriesdir;?></td><td><input type="text" name="seriesdir" value=<?php for($i=0;$i<count($SERIES_DIR);$i++){echo $SERIES_DIR[$i];if($i != (count($SERIES_DIR)-1)) echo ',';}?>></td>
			</tr>
			<tr>
	...
	while($data = mysql_fetch_array($req)){
	        foreach($SERIES_DIR as $series){
	                if(!mb_ereg($series,$data['dir'])){
