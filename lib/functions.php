...
global $LOGIN_SQL;
$db = mysql_connect('localhost',$LOGIN_SQL,$pass);
mysql_select_db($basename,$db);
}

function repertoire($dir){
global $BASE_DIR;
$edir = str_replace($BASE_DIR,home,$dir);
$edir = explode('/',$edir);

echo '<a href="index.php"><img src="images/home.png" alt="home"></a>';
...
//	$redir = $redir.$slash.$edir[$i];
	$redir = str_replace(home,$BASE_DIR,$redir);
	$name = str_replace(home,home.' / ',$edir[$i]);
	$name = $edir[$i];
	echo '<a href="?rep='.urlencode($redir).'">'.$name.'</a> / ';
	}
}
...
function get_folders()
{
	...
	if ($handle = opendir($BASE_DIR )) {
		while (false !== ($file = readdir($handle))) {
			if(( is_dir($BASE_DIR.'/'.$file) || is_link($BASE_DIR.'/'.$file) ) && ( $file != '.' && $file !='..') )			
				$dirs[] = $file;		
	    	}
    	closedir($handle);
	}
...
function check_folder($gfolder){
	global $BASE_DIR;
	$folders = get_folders();
	foreach($folders as $folder){
	
		if( strpos(realpath($BASE_DIR.'/'.$folder), realpath($gfolder) ) === false ){
			return true;
		}
	}
	return false;
...
function is_serie($SERIES_DIR){
        foreach($SERIES_DIR as $dossier){
                $dossier =  str_replace('//','/',$dossier);
                if(substr($dossier,-1) == '/')
                        $dossier = substr($dossier,0,-1);
  //              echo '<script>alert("'.$dossier.' '.$_GET['rep'].'")</script>';
                if(mb_ereg($dossier,urldecode($_GET['rep']))) return true;

        }
//echo '<h1>NNNNNNNNNNNnn</h1>';
        return false;
}
...
function rep($rep){
	global $MOVIES_DIR;
	if (empty($rep)) $dir=$MOVIES_DIR;
	elseif ($rep == '.' or $rep == './') $dir = $MOVIES_DIR;
